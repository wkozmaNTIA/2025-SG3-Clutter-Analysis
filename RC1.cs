...
namespace ClutterAnalysis
{
    internal class RC1
    {
        ...
        private static double[] a_N = new[] { 8.54, 0.056 };
        private static double[] b_N = new[] { 17.57, 6.32 };
        private static double[] c_N = new[] { 0.63, 0.19 };
            ...
            double p, double h__meter, Environment env)
        {
            ...
            double L_clt_NLOS = Math.Max(norm.InverseDistributionFunction(p / 100), 6);
        ...
        public static void CurveSetForFrequency(double f__ghz, double h__meter, Environment env)
        {
            ...
            for (int theta__deg = 0; theta__deg <= 90; theta__deg += 10)
            {
                ...
                {
                    double L_ces__db = RC1.Invoke(f__ghz, theta__deg, p, h__meter, env);
        ...
        public static void CompareWithBoulderMeasurements(Environment env)
        {
            ...
            var pm = new PlotModel()
            {
                ...
                Subtitle = $"RC1 Model; env = {env.ToString()}"
            };
            ...
            for (double p = 0.01; p < 100; p += 0.01)
            {
                double L_ces__db = RC1.Invoke(7.601, 3, p, 2.82, env);
                losses_3deg.Add(L_ces__db);

                L_ces__db = RC1.Invoke(7.601, 6, p, 2.82, env);
            ...
            var pngExporter = new OxyPlot.Wpf.PngExporter { Width = 800, Height = 600 };
            OxyPlot.Wpf.ExporterExtensions.ExportToFile(pngExporter, pm, Path.Combine(@"C:\outputs", "RC1-BoulderMeasurements.png"));
        }
    }
}
